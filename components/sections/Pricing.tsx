...
export default function Pricing() {
  return (
    <section id="pricing" className="px-6 py-28 md:px-10">
      <div className="mb-14">
        <Reveal>
          <p className="mb-3 text-sm uppercase tracking-widest text-accent">Pricing</p>
        </Reveal>
        <AnimatedTitle
        ...
        <Reveal delay={0.1}>
          <p className="mt-4 max-w-xl text-white/70">
            Pick a build tier — or let us run it for you after launch. You own
            everything we build.
          </p>
        </Reveal>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="flex flex-col gap-3">
            {tiers.map((t, i) => (
              <Reveal key={t.name} delay={i * 0.06}>
                <div
                  data-cursor="hover"
                  className={`group relative flex items-center justify-between rounded-2xl border p-6 transition-all duration-500 hover:-translate-y-1 hover:border-accent hover:shadow-[0_0_40px_rgba(61,90,254,0.25)] ${
                    t.popular ? "border-accent bg-accent/5" : "border-white/10 bg-white/[0.03]"
                  }`}
                >
        ...
        <div>
          ...
          <Reveal delay={0.15}>
            <div
              data-cursor="hover"
              className="flex h-full flex-col justify-between rounded-2xl border border-white/10 bg-gradient-to-b from-accent/15 to-transparent p-8 transition-all duration-500 hover:-translate-y-1 hover:border-accent hover:shadow-[0_0_40px_rgba(61,90,254,0.25)]"
            >
              <div>
                ...
                <p className="mt-4 text-sm text-white/70">
                  Hosting, updates, backups, and small changes — handled monthly
